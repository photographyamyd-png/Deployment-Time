...
import { ServiceCapabilitiesExplorer } from "@/components/services/service-capabilities-explorer";
...
export function ServiceFieldCapabilities({ service, subSections }: Props) {
  ...
  const explorer = service.capabilitiesExplorer;
  const useExplorer = service.slug === "drainage-hardscaping" && explorer != null;
  ...
  return (
    <section
      ...
      className={
        useExplorer
          ? "service-field-capabilities service-field-capabilities--explorer"
          : "service-field-capabilities"
      }
      aria-label="Service capabilities and detail"
    >
      {useExplorer ? (
        <ServiceCapabilitiesExplorer sections={subSections} explorer={explorer} />
      ) : (
        <>
          {trust ? (
